    ...
    public class AppNotificationsService : IAppNotificationsService
    {
        ...
        public void SendNotification(AppNotification notification)
        {
            ...
            Task.Run(async () =>
            {
                try
                {
                    if (!notification.NoSound && settingsService.GetNoCache(SettingsConstants.ENABLE_IN_APP_SOUND, true))
                        await PlaySound(notification.Type);
                }
                catch (Exception)
                {
                    settingsService.Set(SettingsConstants.ENABLE_IN_APP_SOUND, false);
            ...
            Task.Run(async () =>
            {
                try
                {
                    if (!notification.NoVibration && settingsService.GetNoCache(SettingsConstants.ENABLE_IN_APP_VIBRATION, true))
                        await StartVibration(notification.Type);
                }
                catch (Exception)
                {
                    settingsService.Set(SettingsConstants.ENABLE_IN_APP_VIBRATION, false);
                    var error = new AppNotification
                    {
                        Type = AppNotificationType.Error,
                        NoVibration = true,
